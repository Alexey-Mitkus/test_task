...
class ServiceDigitalPassportController extends Controller
{
    ...
    {
        SEOMeta::setTitle('Цифровой паспорт');
        // SEOMeta::setDescription('');
        // SEOMeta::setKeywords('');
        
        $user = Auth::user();

        return view('services.digitals.passports.create', compact('user'));
    }

    public function show(Passport $passport, request $request)
    {
        $user = Auth::user();
        ...
        return view('services.digitals.passports.exports.pdf.show', compact('passport', 'user'));
    }

    public function download(Passport $passport, request $request)
    {
        $user = Auth::user();
        $passport = Passport::with(['category', 'organization', 'owner', 'directors', 'teams', 'interests', 'objectives', 'results', 'metrics', 'resources', 'budgets', 'risks', 'plans'])->where('slug', $passport->slug)->first();
        $pdf = \PDF::loadView('services.digitals.passports.exports.pdf.download', compact('passport', 'user'));
        return $pdf->download($passport->slug . '.pdf');
    }
    ...
    public function edit(Passport $passport, request $request)
    {
        ...
        return view('services.digitals.passports.edit', compact('passport', 'user'));
    }
}
