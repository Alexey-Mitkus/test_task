    ...
    <div class="search__form">
        ...
        <template v-if="txtInput.length > 0">
            <div class="search-list">
                <div v-for="item in searchTitle" :key="item.id" class="search-list-item">
                    <a :href="route('community-project.show', item.id)">{{item.name}}</a>
                </div>
            </div>
        </template>
    </div>
...
export default {
    ...
    methods: {
        ...
        getTitle(txtInput) {
            ...
            {
                axios
                    .get(route('api.community-project.search'), {
                        params:{
                            search: txtInput
                        }
                    })
